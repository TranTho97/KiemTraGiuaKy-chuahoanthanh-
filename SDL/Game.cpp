		...
		else
		{
			...
			while (!m_quit)
			{
				...
				while (SDL_PollEvent(&e) != 0)
				{
					...
					{
						player.movement(e);//người chơi chuyển động
					}
					if (e.type == SDL_MOUSEBUTTONDOWN)
					{
						player.shoot(playerLasers);//người chơi bắn
					}
				}
...
{
	//kiểm tra sự va chạm(player, enemies, playerLasers, enemyLasers);
	//kiem tra va cham giua dan nguoi choi va ke thu 
	for (int playerLaser = 0; playerLaser < playerLasers.size(); playerLaser++)
	//for (auto &laser : playerLasers)
	{
		for (int enemy = 0; enemy < enemies.size(); enemy++)
		{
			// phát hiện va chạm xóa đạn người chơi và kẻ thù
